...
- **Docker Compose v2** (`docker compose`) – актуальная версия, встроенная в Docker CLI
...
- **Для всех образов явно задана версия** – обеспечение стабильности и предсказуемости
...
- **Fail2ban на хосте** – защита SSH на уровне ОС, а не внутри контейнера
- **Provisioning в Grafana** – datasource и дашборд настраиваются автоматически при старте
- **Разделение окружений** – через override-файлы, шаблон для `debug` уже подготовлен
- **GitHub Actions** – автоматическая проверка синтаксиса и старта контейнеров при пушах и PR
- **IP-адрес для доступа к админке определяется автоматически** (как пример подстановки переменных в конфиг)
